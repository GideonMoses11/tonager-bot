...
const fs = require("fs");
const db = require("./models"); // This will import db object from models/index.js
const sequelize = db.sequelize; // Get sequelize instance from the imported db object
const User = db.User;
...
sequelize.sync({ force: false })
  ...
  .catch((err) => {
    console.log("Some error occurred during database synchronization:\n", err);
  });
    ...
    if(!user){
        ...
        // ctx.session.messageIds.push(two.message_id);
            ...
            await ctx.reply("\nEnable token withdrawals by clicking and joining the telegram community below\n\n**Please note that accounts not in the community will not be funded**", Markup.inlineKeyboard([
                [
                  Markup.button.url("Join community", "t.me/Tonagerbot")  
                ],
                [
        ...
        await ctx.replyWithPhoto({source: fs.createReadStream('./tm.jpg')}).then(async()=>{
            await ctx.reply(`Hey, ${ctx.from.username}!, Welcome to Tonager\u2764 \n\nKeep tapping to see your balance grow.\n\nTonager is a Decentralized Application which will be listed on Ton blockchain.\n\nGot friends, relatives, co-workers?\nBring them all into the game.\nMore Mates - more coins`,Markup.inlineKeyboard([
                [
